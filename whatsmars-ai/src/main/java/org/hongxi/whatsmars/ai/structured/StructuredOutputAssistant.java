...
public interface StructuredOutputAssistant {
     ...
     * @return 情感分析结果
     */
    @SystemMessage("你是一个情感分析专家，请分析给定文本情感倾向。" +
     ...
     * </p>
     *
     * @param text 包含待办事项的自然语言文本
     * @return 结构化的待办事项列表
     */
    @SystemMessage("你是一个待办事项提取助手。请从文本中提取所有待办事项，包括任务描述、优先级和截止日期。" +
            ...
            "只返回 JSON，不要包含任何其他文字。")
    @UserMessage("请提取以下文本中的待办事项：{{text}}")
    TodoList extractTodos(@V("text") String text);

    /**
     * 将自然语言转换为 SQL 语句
     *
     * @param description 数据查询的自然语言描述
     * @return 结构化的 SQL 信息
     */
    @SystemMessage("你是一个 SQL 专家，请根据用户的自然语言描述生成对应的 SQL 查询语句。" +
            "只返回 SQL 和简要说明，不要解释过程。" +
            "你必须严格以 JSON 格式回复，包含字段: sql(string), explanation(string), tablesUsed(string数组)。" +
            "只返回 JSON，不要包含任何其他文字。")
    @UserMessage("请为以下需求生成 SQL：{{description}}")
    SqlResult generateSql(@V("description") String description);
